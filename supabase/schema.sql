...
CREATE TABLE IF NOT EXISTS public.market_analysis_cache (
  id UUID DEFAULT uuid_generate_v4() PRIMARY KEY,
  location_key TEXT NOT NULL, -- ZIP code or normalized city name
  property_type TEXT,
  data_type TEXT NOT NULL CHECK (data_type IN ('sale', 'rental', 'all')),
  raw_data JSONB NOT NULL, -- Store complete API response
  parsed_data JSONB NOT NULL, -- Normalized MarketData format
  source TEXT NOT NULL DEFAULT 'rentcast', -- API source
  fetched_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  expires_at TIMESTAMP WITH TIME ZONE NOT NULL,
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  -- Composite unique constraint for location + type
