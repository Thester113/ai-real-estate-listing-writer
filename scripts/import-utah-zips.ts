#!/usr/bin/env tsx
/**
 * Import Utah ZIP codes including 84005
 */
...
async function importUtahData() {
  ...
  console.log('🌐 Downloading Utah ZIP code data...')

  // Download and grep for Utah (UT) records
  const { stdout } = await execAsync(
    `curl -s "${REDFIN_ZIP_URL}" | gunzip | grep -E '(^"PERIOD|\\tUT\\t)' | head -5000`
  )

  const lines = stdout.trim().split('\n')
  const headers = lines[0].split('\t').map(h => h.replace(/"/g, ''))

  console.log(`📊 Processing ${lines.length - 1} Utah records...`)
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    // Get the most recent period for each ZIP/property type combo
    records.push({
      zip_code: zipCode,
  ...
  console.log('🎉 Utah data import complete!')
  console.log(`Total Utah records imported: ${processedCount}`)
}

importUtahData().catch((error) => {
